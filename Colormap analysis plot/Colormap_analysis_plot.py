...
def colormap_plot(data_score_df, var_names, N_groups, group_names = [], annot_flag = True, thr_flag = True, score_thr = 0, colorbar_flag = True, fig_title = "", yticks_flag = True, save_flag = False, save_dir = ""):
    ...
    if colorbar_flag == True:
        ...
        f, axs = plt.subplots(1, 1, gridspec_kw={'width_ratios':[1]}, figsize=(10, 15))
        ax1 = axs
    ...
    if colorbar_flag == True:
        ...
        h1 = sns.heatmap(data_heatmap_df, annot=annotations, fmt="s", cmap=my_cmap, vmin=min(color_separation)-color_sep_mid, vmax=max(color_separation)+color_sep_mid, annot_kws={"size":15}, xticklabels=xticks, linewidths=1, ax=ax1, cbar=colorbar_flag)
